import './partials/veloura-raed-glass-v82';
...
/* ========================================================================
   Veloura V79 — single Salla search hydration bridge

   Only search internals are touched:
   - Header/detached search internals stay transparent.
   - Standalone search uses the real secondary site color.
   - No menu geometry and no add-to-cart click handling.
   ======================================================================== */
const initVelouraSearchSurfaceV79 = (() => {
  const STYLE_ID = 'veloura-search-surface-v79';
  ...
  const shadowCss = `
    ...
    :host([data-veloura-search-inline='true']),
    :host([data-veloura-search-inline='true']) form,
    :host([data-veloura-search-inline='true']) input,
    :host([data-veloura-search-inline='true']) .s-search-input,
    :host([data-veloura-search-inline='true']) .s-search-container,
    :host([data-veloura-search-inline='true']) .s-search-wrapper,
    :host([data-veloura-search-inline='true']) [part~='input'],
    :host([data-veloura-search-inline='true']) [part~='form'],
    :host([data-veloura-search-inline='true']) [part~='container'] {
      background: transparent !important;
    ...
    :host([data-veloura-search-inline='false']) form,
    :host([data-veloura-search-inline='false']) input,
    :host([data-veloura-search-inline='false']) .s-search-input,
    :host([data-veloura-search-inline='false']) [part~='input'],
    :host([data-veloura-search-inline='false']) [part~='form'] {
      background: var(--veloura-search-runtime-bg) !important;
      background-color: var(--veloura-search-runtime-bg) !important;
      color: var(--veloura-search-runtime-text) !important;
      border-color: transparent !important;
      box-shadow: none !important;
    }

    :host([data-veloura-search-dark='true']) .s-search-results,
    :host([data-veloura-search-dark='true']) .s-search-results-wrapper,
    :host([data-veloura-search-dark='true']) .s-search-result,
    :host([data-veloura-search-dark='true']) .s-search-result-item,
    :host([data-veloura-search-dark='true']) [part~='results'],
    :host([data-veloura-search-dark='true']) [part~='panel'] {
      background: var(--veloura-search-runtime-bg) !important;
      background-color: var(--veloura-search-runtime-bg) !important;
      color: var(--veloura-search-runtime-text) !important;
      border-color: transparent !important;
    ...
    .s-search-input::placeholder {
      color: var(--veloura-search-runtime-muted) !important;
      opacity: 1 !important;
    }
  `;
  ...
  function paint(host) {
    ...
    const dark = isDark();
    const inline = isInline(host);
    const background = dark
      ? cssVar('--veloura-dark-secondary-bg', '#010612')
      : cssVar('--veloura-site-second-bg', '#f4f5f7');
    const text = dark
      ? cssVar('--veloura-dark-primary-text', '#ffffff')
      : 'inherit';
    const muted = dark
      ? cssVar('--veloura-dark-secondary-text', '#d7e0e8')
      : '#64748b';

    host.setAttribute('data-veloura-search-inline', inline ? 'true' : 'false');
    host.setAttribute('data-veloura-search-dark', dark ? 'true' : 'false');

    setImportant(host, '--veloura-search-runtime-bg', background);
    setImportant(host, '--veloura-search-runtime-text', text);
    setImportant(host, '--veloura-search-runtime-muted', muted);
    ...
    if (inline) {
      ...
      [
        ...
        '--color-grey-light',
        '--color-grey-lighter',
      ].forEach((name) => setImportant(host, name, background));
    }
    ...
    `).forEach((element) => {
      ...
      if (inline) {
        ...
        setImportant(element, 'background', background);
        setImportant(element, 'background-color', background);
      }
    });

    if (!observedRoots.has(root)) {
      const observer = new MutationObserver(() => schedule(host));
  ...
  function scan(scope = document) {
    if (scope.matches?.('salla-search')) schedule(scope);
    scope.querySelectorAll?.('salla-search').forEach(schedule);
  }

  function syncAll() {
    document.querySelectorAll('salla-search').forEach(schedule);
  }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initVelouraSearchSurfaceV79, {
    once: true,
  });
} else {
